...
// set events
func setInvokeErrorEvent(stub shim.ChaincodeStubInterface, err error) {
	_ = stub.SetEvent(EVTINVOKEERR, []byte(fmt.Sprintf("INVOKE ERROR: %+v", err)))
}

func setPingEvent(stub shim.ChaincodeStubInterface, count int) {
	_ = stub.SetEvent(EVTPONG, []byte(fmt.Sprintf("PING: %d", count)))
}

func setPongEvent(stub shim.ChaincodeStubInterface, count int) {
...
func (t *SimpleChaincode) invokefunc(stub shim.ChaincodeStubInterface, function string, args []string) ([]byte, error) {
...
func (t *SimpleChaincode) queryfunc(stub shim.ChaincodeStubInterface, function string, args []string) ([]byte, error) {
...
func (t *SimpleChaincode) createAsset(stub shim.ChaincodeStubInterface, args []string) ([]byte, error) {
	...
	stateOut, ok := asMap(deepMerge(map[string]interface{}(argsMap),
		make(map[string]interface{})))
	if !ok {
		err = errors.New("createAsset: stateOut not a map shape")
		setInvokeErrorEvent(stub, err)
		return nil, err
	}
	...
	am := ArgsMap(stateOut)
	amp := &am
	noncompliant, err := amp.executeRules(&alerts)
	if err != nil {
...
func (t *SimpleChaincode) updateAsset(stub shim.ChaincodeStubInterface, args []string) ([]byte, error) {
	...
	if !ok {
		err = errors.New("updateAsset: stateOut not a map shape")
		setInvokeErrorEvent(stub, err)
		return nil, err
	}
