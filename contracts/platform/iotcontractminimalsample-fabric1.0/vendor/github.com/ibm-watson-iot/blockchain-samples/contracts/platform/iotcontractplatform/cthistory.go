...
func (c *AssetClass) DeleteAssetStateHistory(stub shim.ChaincodeStubInterface, args []string) ([]byte, error) {
	...
	for iter.HasNext() {
		...
		err = stub.DelState(res.Key)
		if err != nil {
			err = fmt.Errorf("DeleteAssetStateHistory DelState for asset %s failed: %s ", res.Key, err)
			log.Errorf(err.Error())
			return nil, err
		}
	}
...
func (c *AssetClass) ReadAssetStateHistory(stub shim.ChaincodeStubInterface, args []string) ([]byte, error) {
	...
	for iter.HasNext() {
		...
		if err != nil {
			err = fmt.Errorf("ReadAssetStateHistory unmarshal %s failed: %s", assetBytes.Key, err)
			log.Errorf(err.Error())
			return nil, err
		}
