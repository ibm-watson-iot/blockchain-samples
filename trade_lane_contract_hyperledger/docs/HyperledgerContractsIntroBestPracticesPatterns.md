...
Maps, on the other hand, reflect exactly what was present in the event or state and offer a very flexible way to deal with incoming data. It is even possible to validate incoming events against the contract payload schema, although trade lane does not yet implement that check. 
