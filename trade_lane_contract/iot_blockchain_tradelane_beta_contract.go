...
// these must match schema JSON tags
...
func (t *SimpleChaincode) init(stub *shim.ChaincodeStub, args []string) ([]byte, error) {
	...
	if err != nil {
		log.Criticalf("Version argument unmarshal failed: %s", err)
		return nil, err 
	}
...
func (t *SimpleChaincode) Run(stub *shim.ChaincodeStub, function string, args []string) ([]byte, error) {
	...
	if function == "init" {
		...
		// Deletes a list of properties from an asset by ID from the ledger
		...
		// change whether a missing asset is created when
        // an update for it is sent, true says create, 
        // false says error
		return nil, t.setCreateOnUpdate(stub, args)
	}
	err := errors.New(fmt.Sprintf("Invoke received unknown invocation: %s", function))
    log.Warning(err)
	return nil, err
}
...
func (t *SimpleChaincode) Query(stub *shim.ChaincodeStub, function string, args []string) ([]byte, error) {
	...
	if function == "readAsset" {
		...
		// gets the state for every managed assetID as a JSON struct
		...
		// gets the state for the chaincode itself as a JSON struct
		return t.readContractState(stub, args)
	}
	err := errors.New(fmt.Sprintf("Query received unknown invocation: %s", function))
    log.Warning(err)
	return nil, err
}
...
func (t *SimpleChaincode) createAsset(stub *shim.ChaincodeStub, args []string) ([]byte, error) {
    ...
    var argsMap ArgsMap
	var event interface{}   // JSON unmarshal target
    var found bool
	var err error
    ...
    if found {
        err := errors.New(fmt.Sprintf("createAsset arg asset %s already exists!", assetID))
        log.Error(err)
        return nil, err
    }
    ...
    if err != nil {
        err := errors.New(fmt.Sprintf("createAsset state for assetID %s failed to marshal", assetID))
        log.Error(err)
        return nil, err
    }
    ...
    if err != nil {
        err := errors.New(fmt.Sprintf("createAsset arg asset %s got error from PURSTATE: %s", assetID, err))
        log.Error(err)
        return nil, err
    } else {
    ...
    if err != nil {
        err := errors.New(fmt.Sprintf("createAsset asset %s failed to write asset state: %s", assetID, err))
        log.Critical(err)
        return nil, err 
    }
    // save state history
    ...
    if err != nil {
        err := errors.New(fmt.Sprintf("createAsset asset %s state history save failed: %s", assetID, err))
        log.Critical(err)
        return nil, err 
    }
    ...
    if err != nil {
        err := errors.New(fmt.Sprintf("createAsset asset %s recent state save failed: %s", assetID, err))
        log.Critical(err)
        return nil, err 
    }
...
func (t *SimpleChaincode) updateAsset(stub *shim.ChaincodeStub, args []string) ([]byte, error) {
	var assetID string                 // asset ID
	var argsMap ArgsMap                // incoming asset state from argument, use interface so that only present data is used to change state
	var event interface{}            // incoming asset state from argument, use interface so that only present data is used to change state
	var ledgerMap ArgsMap              // ledger asset state from argument, use interface so that only present data is used to change state
	var ledgerBytes interface{}        // ledger asset state from argument, use interface so that only present data is used to change state
	var found bool                     // used when looking in map
	var err error
    var timeIn time.Time
    ...
    stateJSON, err := json.Marshal(stateOut)
    if err != nil {
        log.Errorf("updateAsset assetID %s marshal failed: %s", assetID, err)
        return nil, err
    }
    err = stub.PutState(assetID, []byte(stateJSON))
    if err != nil {
        log.Errorf("updateAsset assetID %s PUTSTATE failed: %s", assetID, err)
        return nil, err
    } else {
        log.Infof("Asset %s state successfully written to ledger", assetID)
    }
    err = pushRecentState(stub, string(stateJSON))
    if err != nil {
        log.Errorf("updateAsset assetID %s push to recentstates failed: %s", assetID, err)
        return nil, err
    }
    // add history state
    err = updateStateHistory(stub, assetID, string(stateJSON))
    if err != nil {
        log.Errorf("updateAsset assetID %s push to history failed: %s", assetID, err)
        return nil, err
    }
...
func (t *SimpleChaincode) deleteAsset(stub *shim.ChaincodeStub, args []string) ([]byte, error) {
    ...
    found = assetIsActive(stub, assetID)
    if !found {
        err = errors.New(fmt.Sprintf("deleteAsset assetID %s does not exist", assetID))
        log.Error(err)
        return nil, err
    }
    ...
    if err != nil {
        err := errors.New(fmt.Sprintf("deleteAsset asset %s failed to remove asset from contract state: %s", assetID, err))
        log.Critical(err)
        return nil, err 
    }
    // save state history
    ...
    if err != nil {
        err := errors.New(fmt.Sprintf("deleteAsset asset %s state history delete failed: %s", assetID, err))
        log.Critical(err)
        return nil, err 
    }
    // push the recent state
    ...
    if err != nil {
        err := errors.New(fmt.Sprintf("deleteAsset asset %s recent state removal failed: %s", assetID, err))
        log.Critical(err)
        return nil, err 
    }
...
func (t *SimpleChaincode) deletePropertiesFromAsset(stub *shim.ChaincodeStub, args []string) ([]byte, error) {
    ...
    found = assetIsActive(stub, assetID)
    if !found {
        err = errors.New(fmt.Sprintf("deletePropertiesFromAsset assetID %s does not exist!", assetID))
        log.Error(err)
        return nil, err
    }
    ...
    assetBytes, err := stub.GetState(assetID)
    if err != nil {
        log.Errorf("deletePropertiesFromAsset AssetID %s GETSTATE failed: %s", assetID, err)
        return nil, err
    }
    ...
    err = json.Unmarshal(assetBytes, &ledgerBytes)
    if err != nil {
        log.Errorf("deletePropertiesFromAsset AssetID %s unmarshal failed: %s", assetID, err)
        return nil, err
    }
    ...
    ledgerMap, found = ledgerBytes.(map[string]interface{})
    if !found {
        log.Errorf("deletePropertiesFromAsset AssetID %s LEDGER state is not a map shape", assetID)
        return nil, err
    }
    ...
    if err != nil {
        log.Errorf("deletePropertiesFromAsset AssetID %s marshal failed: %s", assetID, err)
        return nil, err
    }
    ...
    err = stub.PutState(assetID, []byte(stateJSON))
    if err != nil {
        log.Errorf("deletePropertiesFromAsset AssetID %s PUTSTATE failed: %s", assetID, err)
        return nil, err
    }
    err = pushRecentState(stub, string(stateJSON))
    if err != nil {
        log.Errorf("deletePropertiesFromAsset AssetID %s push to recentstates failed: %s", assetID, err)
        return nil, err
    }
    ...
    err = updateStateHistory(stub, assetID, string(stateJSON))
    if err != nil {
        log.Errorf("deletePropertiesFromAsset AssetID %s push to history failed: %s", assetID, err)
        return nil, err
    }
...
func (t *SimpleChaincode) deleteAllAssets(stub *shim.ChaincodeStub, args []string) ([]byte, error) {
    ...
    if err != nil {
        log.Errorf("deleteAllAssets failed to get the active assets: %s", err)
    }
    for i := range aa {
        assetID = aa[i]
        ...
        err = stub.DelState(assetID)
        if err != nil {
            log.Errorf("deleteAllAssets arg %d assetID %s failed DELSTATE", i, assetID)
        return nil, err
        }
        // remove asset from contract state
        err = removeAssetFromContractState(stub, assetID)
        if err != nil {
            err := errors.New(fmt.Sprintf("deleteAllAssets asset %s failed to remove asset from contract state: %s", assetID, err))
            log.Critical(err)
            return nil, err 
        }
        // save state history
        err = deleteStateHistory(stub, assetID)
        if err != nil {
            err := errors.New(fmt.Sprintf("deleteAllAssets asset %s state history delete failed: %s", assetID, err))
            log.Critical(err)
            return nil, err 
        }
    }
...
func (t *SimpleChaincode) readAsset(stub *shim.ChaincodeStub, args []string) ([]byte, error) {
    ...
    found = assetIsActive(stub, assetID)
    if !found {
        err := errors.New(fmt.Sprintf("readAsset arg asset %s does not exist!", assetID))
        log.Error(err)
        return nil, err
    }
...
func (t *SimpleChaincode) readAllAssets(stub *shim.ChaincodeStub, args []string) ([]byte, error) {
	...
	if len(args) > 0 {
        err = errors.New("readAllAssets Too many arguments. Expecting none.")
		log.Error(err)
		return nil, err
	}
    ...
    aa, err := getActiveAssets(stub)
    if err != nil {
        log.Errorf("readAllAssets failed to get the active assets: %s", err)
    }
    ...
    if err != nil {
        log.Errorf("readallAssets failed to marshal results: %s", err)
        return nil, err
    }
...
func (t *SimpleChaincode) readAssetHistory(stub *shim.ChaincodeStub, args []string) ([]byte, error) {
	...
	if len(args) != 1 {
        err = errors.New("readAssetHistory Too many arguments. Expecting a single JSON encoded object with assetID and count.")
		log.Error(err)
		return nil, err
	}
    ...
    err = json.Unmarshal(requestBytes, &request)
    if err != nil {
        log.Error("readAssetHistory failed to unmarshal arg")
        return nil, err
    }
    ...
    found = assetIsActive(stub, assetID)
    if !found {
        err := errors.New(fmt.Sprintf("readAssetHistory arg asset %s does not exist!", assetID))
        log.Error(err)
        return nil, err
    }
    ...
    if err != nil {
        log.Errorf("readAssetHistory assetID %s failed readStateHistory: %s", assetID, err)
        return nil, err
    }
    ...
    var hStatesOut []interface{} = make([]interface{}, 0, olen) 
    for i := 0; i < olen; i++ {
        var obj interface{}
...
// Query returns state of chaincode (all assets)
func (t *SimpleChaincode) readContractState(stub *shim.ChaincodeStub, args []string) ([]byte, error) {
	...
	if err != nil {
        log.Errorf("readContractState failed GETSTATE")
        return nil, err
	}
...
func (t *SimpleChaincode) readContractObjectModel(stub *shim.ChaincodeStub, args []string) ([]byte, error) {
	...
	if err != nil {
        err := errors.New(fmt.Sprintf("JSON Marshal failed for get contract object model empty state: %+v with error [%s]", state, err))
        log.Error(err)
		return nil, err
	}
...
func (t *SimpleChaincode) setLoggingLevel(stub *shim.ChaincodeStub, args []string) (error) {
	...
	if err != nil {
        err = errors.New("setLoggingLevel failed to unmarshal arg: " + args[0])
		log.Error(err)
		return err
    }
    ...
    err = errors.New("Unknown Logging level: " + level.Level)
    log.Error(err)
    return err
}
...
func (t *SimpleChaincode) setCreateOnUpdate(stub *shim.ChaincodeStub, args []string) (error) {
    ...
    var err error
    if len(args) != 1 {
        err = errors.New("setCreateOnUpdate Incorrect number of arguments. Expecting a JSON encoded createOnUpdate boolean.")
		log.Error(err)
		return err
    }
    err = json.Unmarshal([]byte(args[0]), &createOnUpdate)
	if err != nil {
        err = errors.New("setCreateOnUpdate failed to unmarshal arg: " + args[0])
		log.Error(err)
		return err
    }
...
func PUTcreateOnUpdate(stub *shim.ChaincodeStub, createOnUpdate CreateOnUpdate) (err error) {
	...
	if err != nil {
        err = errors.New("PUTSTATE createOnUpdate failed")
		log.Error(err)
		return err
    }
    return nil
...
func canCreateOnUpdate(stub *shim.ChaincodeStub) (bool) {
	...
	if err != nil {
        err = errors.New("GETSTATE for canCreateOnUpdate failed")
		log.Error(err)
		return true  // true is the default
    }
    err = json.Unmarshal(createOnUpdateBytes, &createOnUpdate)
	if err != nil {
        err = errors.New("canCreateOnUpdate failed to marshal")
		log.Error(err)
		return true  // true is the default
    }
