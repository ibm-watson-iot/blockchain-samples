...
const RECENTSTATESKEY string = "RecentStatesKey"	// store contract state such as asset list
...
const MAX_RECENT_STATES int = 20 

func GETRecentStatesFromLedger(stub *shim.ChaincodeStub) (RecentStates, error) {
    var state RecentStates = RecentStates{make([]string, 0, MAX_RECENT_STATES)}
    var err error
    ...
    if state.RecentStates == nil || len(state.RecentStates) == 0 {
        state.RecentStates = make([]string, 0, MAX_RECENT_STATES)
    }
...
func clearRecentStates(stub *shim.ChaincodeStub) (error) {
    ...
    rstates.RecentStates = make([]string, 0, MAX_RECENT_STATES)
...
func pushRecentState (stub *shim.ChaincodeStub, state string) (error) {
    ...
    if err != nil {
        ...
        if len(rstate.RecentStates) < MAX_RECENT_STATES {
...
func readRecentStates(stub *shim.ChaincodeStub) ([]byte, error) {
	var err error
    var rstate RecentStates
    var rstateOut []interface{} = make([]interface{}, 0, MAX_RECENT_STATES) 
