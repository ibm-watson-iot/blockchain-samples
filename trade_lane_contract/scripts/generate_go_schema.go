...
func sampleType(obj map[string]interface{}, elementName string) (interface{}) {
    ...
    switch t {
        ...
        case "string" :
            if strings.ToLower(elementName) == "timestamp" {
                ...
                if found && len(enum) > 0 {
                    ...
                    // description is a good choice for sample data since it explains the prospective contents
        ...
        case "boolean" :
            return true
        case "array" :
            return arrayFromSchema(obj["items"].(map[string]interface{}), elementName)
        case "object" : {
